...
function FOM_ChatCommandHandler(msg)
	...
	if ( cmd == "feed" ) then
		if (foodString == "") then
			...
			local inputFoods = { };
			for itemLink in string.gfind(foodString, "%[.*%]") do
	...
	for bagNum = 0, 4 do
		...
		-- skip bags that can't contain food
			...
			for itemNum = 1, bagSize do
				...
				-- GFWUtils.Print(' ' .. tostring(itemLink) .. ' ' .. tostring(foodName) );
				-- if ( itemLink == foodName ) then
				-- 	return bagNum, itemNum;
				-- end
				if (itemLink) then
					local itemName = FOM_NameFromLink(itemLink);
...
function FOM_PetAbandon()
	...
	FOM_Original_PetAbandon();  --Monteo swap
